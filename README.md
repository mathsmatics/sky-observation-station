...
左上角 `DBG` 是调试面板，用于排查布局、数据加载、时间输入、skyview、fallback、redraw、rollback、时区偏移和行星计算状态；普通使用时可以关闭。遇到刷新或布局异常时，可以复制 debug 面板内容给维护者排查。远古或远未来日期允许输入，但当前项目没有完整岁差、章动、恒星自行和高精度行星历表，相关显示属于近似天文馆效果，不等同于科研级古天文历表。早期年份的 UTC 换算会遵循 IANA 历史时区规则，可能出现不是现代固定东八区的偏移。
