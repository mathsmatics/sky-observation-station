...
5.4.6 在不改变现有功能和算法逻辑的前提下继续拆分 `src/app.ts`：天体搜索 UI、天体信息格式化、固定星空历元同步、行星 overlay、参考覆盖层 overlay、中国传统天区边界 overlay、对象点击拾取、中文星官主 overlay、UI 事件绑定、应用动画帧循环、时间输入动作、观测地点更新和视角模式切换分别迁入 `src/ui/object-search.ts`、`src/ui/object-info.ts`、`src/sky/epoch-frame.ts`、`src/sky/planet-overlay.ts`、`src/sky/reference-overlays.ts`、`src/sky/traditional-regions-overlay.ts`、`src/sky/object-picking.ts`、`src/sky/culture-overlays.ts`、`src/ui/event-bindings.ts`、`src/runtime/app-animation.ts`、`src/time/time-input-actions.ts`、`src/time/observer-location.ts`、`src/sky/view-mode-switching.ts`。`app.ts` 继续保留状态、D3-Celestial 生命周期、星图主配置和装配组织权。Debug 面板新增星图刷新耗时，显示固定图层同步、Celestial redraw 和总耗时。中西星官同时显示时，中国传统天区边界只临时禁用并变暗，不再改写用户保存的开关状态。
