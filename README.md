...
5.4.4 在不改变现有功能和算法逻辑的前提下继续拆分 `src/app.ts`：天体搜索 UI、天体信息格式化、固定星空历元同步分别迁入 `src/ui/object-search.ts`、`src/ui/object-info.ts`、`src/sky/epoch-frame.ts`。`app.ts` 继续保留状态、D3-Celestial 生命周期、事件装配和高耦合绘制入口。Debug 面板新增星图刷新耗时，显示固定图层同步、Celestial redraw 和总耗时。中西星官同时显示时，中国传统天区边界只临时禁用并变暗，不再改写用户保存的开关状态。
