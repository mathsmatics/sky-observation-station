 ...
 * 真实星空观测台 4.8 —— 用户可配置文件
 * ------------------------------------------------------------
 * 修改本文件后，刷新 index.html 即可生效；不需要重新打包或构建。
...
window.RSO_CONFIG = {
  ...
  western: {
    line: {
      ...
      width: [1.15, 1.00, 0.85],
      opacity: [0.80, 0.72, 0.62]
    },
    name: {
      fill: "#cce9ff",
      font: [
  ...
  dualCultureLines: {
    ...
    maxOffset: 2.10,                     // 单侧最大偏移，避免过度偏离真实星位
    ...
    haloExtraWidth: 1.30,                // 描边比彩色线额外增加的宽度（px）
    western: {stroke: "#82b9df", width: 1.00, opacity: 0.68},
    chinese: {stroke: "#f08d63", width: 0.98, opacity: 0.68}
  },

  /** 选中天体信息 */
  objectInfo: {
    cultureNoteMagnitudeLimit: 2.10      // 视星等不大于该值的恒星尝试显示中西文化简述
  },
  ...
  projectionZoom: {
    airy:1, orthographic:1, stereographic:1,
    azimuthalEquidistant:1, azimuthalEqualArea:1,
    aitoff:1, hammer:1, mollweide:1, winkel3:1,
    equirectangular:1, healpix:1, mercator:1,
    robinson:1, sinusoidal:1
  }
};
