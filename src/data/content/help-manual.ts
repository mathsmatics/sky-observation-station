  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "paragraph",
          html: "只有真正命中恒星、太阳系天体、深空天体、星座名或中国星官名时，程序才显示浮窗和十字标记。单击空白天区只会清除当前选择，不再把任意天球坐标伪装成一个对象。浮窗位于星图控件上层，支持鼠标选择文字、滚动长文和打开资料来源链接；在浮窗中操作不会穿透到星图拖动。",
        },
        {
          type: "table",
          headers: ["字段", "含义"],
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "paragraph",
          html: "这个公式把赤道坐标转换为本地高度角。<code>φ</code> 是观测纬度，<code>δ</code> 是赤纬，<code>h</code> 是高度角。高度角大于 0° 通常表示天体在几何地平线上方。",
        },
        {
          type: "table",
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "paragraph",
          html: "5.5.5 将 <code>src/data/culture-notes.ts</code> 整理成单一的可选文化增强数据库。西方部分覆盖工程中的 88 个现代星座：古典星座说明人物、事件、象征和相邻星座在同一故事中的牵连；拉卡伊、赫维留及大航海时代新增星座则说明制图者、仪器、动物观察和时代背景，不强行套入希腊神话。",
        },
        {
          type: "paragraph",
          html: "中国部分按三垣、二十八宿、四象和近南极星区组织工程内可点击的星官。每条先解释名称中的人物、官职、器物或场所，再说明它在同宿/同垣中的职能关系。例如天市垣不是普通集市：帝座代表最高权力，候负责候望与监察，斗、斛、帛度体现度量衡，列肆和车肆是交易区，左右垣以列国围成市场边界；井宿南部则把军市、野鸡、天狼和弧矢连成军营防卫叙事。",
        },
        {
          type: "paragraph",
          html: "古籍栏只显示能够对应到具体星官的原文，并同时给出来源链接。例如《步天歌》写军市与野鸡：<q>军市团圆十三星，中有一个野鸡精</q>；写弧矢射狼：<q>左边九个弯弧弓，一矢拟射顽狼胸</q>。《史记·天官书》对北斗则说：<q>斗为帝车，运于中央，临制四乡</q>。界面不再用“请结合某书阅读”代替实际材料。",
        },
        {
          type: "table",
          headers: ["观测月份/时间的定义", "说明"],
          rows: [
            ["参考时刻", "默认按观测地当地标准时约 21:00，目标接近上中天或处于较高位置时估算。不是指整晚任何时刻都同样适合。"],
            ["月份移动", "恒星天空每天约提前 4 分钟；因此每往后一个月，同一星区约提前 2 小时到达相近位置。若某星座 1 月 21:00 合适，2 月大约 19:00、12 月大约 23:00 可看到相近朝向。"],
            ["中国星官", "中国星官也使用其标签中心赤经进行同样的 21:00 估算；它们不是另一套物理天空，所以观测时间规则与西方星座相同。"],
            ["南北半球", "月份主要由赤经决定，但能否升起、最高高度和季节体感取决于纬度。浮窗会按当前观测地点选择南/北半球提示；采用夏令时的地点还需把钟表时间考虑约 1 小时偏移。"],
          ],
        },
        {
          type: "warning",
          html: "分野只在条目能够给出具体文献传统和来源时显示。若不同文献的地域分配无法在当前资料中可靠区分，该条目直接不显示分野，不再为每个星官重复一段没有来源的通用免责声明。",
        },
        {
          type: "note",
          html: "星座连线是便于认星的图案；星座边界是现代天文学划分全天区域的边界。边界显示如果出现断裂、极区跳线或跨 0° 经线异常，应优先看 debug / 控制台诊断，不应手工把边界画成想象中的闭合图案。",
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        {
          type: "formula",
          html: "m_1 - m_2 = -2.5\u00a0\u006c\u006f\u0067_{10}(F_1/F_2)",
        },
        {
          type: "table",
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ...
            [
              "木星",
              "约 400″",
              "约 0.11°，低于 1° 视觉阈值，但不是 VSOP87 级。",
            ],
            [
              "土星",
              "约 600″",
              "约 0.17°，是本组中误差较大的行星之一，但区间内视觉星图仍可接受。",
            ],
            ["天王星", "约 50″", "区间内视觉位置可用。"],
            ["海王星", "约 10″", "区间内视觉位置可用。"],
          ],
        ...
        {
          type: "note",
          html: "网页星图里的“肉眼几乎不可见”通常更接近 0.5°–1° 的视觉阈值；真正天文观测中的肉眼角分辨率可接近 1′，标准更严格。不要把页面上看起来接近误解为专业星历级精度。",
        },
        {
          type: "warning",
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "paragraph",
          html: "控制台中的星座边界诊断会检查边界线数量、相邻星座配对、缺少边界的星座、疑似重复边界、悬空端点、跨日期线片段和极区片段。它用于判断边界异常是数据问题、跨 0° 经线问题、极区投影问题，还是渲染裁剪问题。",
        },
        {
          type: "paragraph",
